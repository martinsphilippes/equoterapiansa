...
export async function announcementsFor(user: UserProfile, practitionerIds: string[] = []): Promise<Announcement[]> {
  const all = mapDocs(await Collections.announcements().orderBy("createdAt", "desc").limit(200).get());
