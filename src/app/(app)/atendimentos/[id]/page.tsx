...
export default async function SessionPage({ params, searchParams }: { params: Params<{ id: string }>; searchParams: SearchParams }) {
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title={`Atendimento · ${isoToBR(s.date)} ${s.time}`} subtitle={<Link href={`/praticantes/${p.id}/atendimentos`} className="text-brand-700 hover:underline">{p.name}</Link>} actions={canEdit && <Link href={`/atendimentos/${id}?editar=1`} className="text-sm text-brand-700 hover:underline">Editar</Link>} />
      <Card>
