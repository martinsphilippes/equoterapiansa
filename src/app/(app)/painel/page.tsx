    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <Link href="/colaboradores"><Stat label="Ativos" value={d.collaborators.active} icon={<Users className="h-4 w-4" />} /></Link>
          <Link href="/jornada/equipe"><Stat label="Presentes hoje" value={d.collaborators.presentToday} hint={`de ${d.collaborators.expectedToday} previstos`} tone="green" icon={<CheckCircle2 className="h-4 w-4" />} /></Link>
          <Link href="/jornada/equipe"><Stat label="Ausentes" value={d.collaborators.absentToday} tone={d.collaborators.absentToday ? "red" : "default"} icon={<XCircle className="h-4 w-4" />} /></Link>
          <Link href="/pagamentos"><Stat label="Horas do mês" value={minutesToHM(d.collaborators.monthMinutes)} icon={<Clock className="h-4 w-4" />} /></Link>
          <Link href={`/pagamentos?mes=${d.prevCompetence}`} className="col-span-2 md:col-span-1"><Stat label="Pagamentos pendentes" value={d.collaborators.pendingPayments} hint={competenceLabel(d.prevCompetence)} tone={d.collaborators.pendingPayments ? "amber" : "green"} icon={<Wallet className="h-4 w-4" />} /></Link>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <Link href="/praticantes"><Stat label="Ativos" value={d.practitioners.active} tone="primary" icon={<UserRound className="h-4 w-4" />} /></Link>
          <Link href="/agenda"><Stat label="Atendimentos hoje" value={d.practitioners.appointmentsToday} hint={`${d.practitioners.pendingToday} a realizar`} icon={<CalendarDays className="h-4 w-4" />} /></Link>
          <Link href="/agenda"><Stat label="Presentes" value={d.practitioners.presentToday} tone="green" icon={<CalendarCheck className="h-4 w-4" />} /></Link>
          <Link href="/agenda"><Stat label="Faltas" value={d.practitioners.missedToday} tone={d.practitioners.missedToday ? "red" : "default"} icon={<XCircle className="h-4 w-4" />} /></Link>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Card title="Próximos atendimentos" action={<Link href="/agenda" className="text-sm font-semibold text-primary-600 hover:underline">Agenda</Link>} className="lg:col-span-2 p-0">
          ...
          <Card title={<span className="flex items-center gap-2">Pendências {pend > 0 && <span className="inline-flex h-5 min-w-5 px-1.5 items-center justify-center rounded-full bg-warning text-white text-[11px] font-bold">{pend}</span>}</span>}>
            ...
            {(d.practitioners.assessmentsPending.length > 0 || d.operation.reportsPending.length > 0) && (
              <ul className="mt-3 pt-3 border-t border-border text-sm space-y-1">
                {d.practitioners.assessmentsPending.slice(0, 5).map((p) => <li key={p.id}><Link href={`/praticantes/${p.id}/avaliacoes`} className="font-medium text-primary-600 hover:underline">{p.name}</Link> <span className="text-ink-500">· avaliação</span></li>)}
                {d.operation.reportsPending.slice(0, 5).map((p) => <li key={p.id}><Link href={`/praticantes/${p.id}/relatorios`} className="font-medium text-primary-600 hover:underline">{p.name}</Link> <span className="text-ink-500">· relatório</span></li>)}
              </ul>
            )}
          </Card>
          <Card title="Comunicados" action={<Link href="/comunicados" className="text-sm font-semibold text-primary-600 hover:underline">Ver todos</Link>}>
            {unread.length === 0 ? <p className="text-sm text-ink-500">Nenhum comunicado novo.</p> : <ul className="text-sm space-y-2">{unread.map((a) => <li key={a.id}><Link href="/comunicados" className="font-semibold hover:underline">{a.title}</Link></li>)}</ul>}
          </Card>
        </div>
      </section>
    </div>
    ...
    <div className="space-y-5">
      ...
      {role === "professional" && (
        <Card title="Meus praticantes">
          {mine.length === 0 ? <p className="text-sm text-ink-500">Nenhum praticante atribuído a você ainda.</p> : <ul className="text-sm grid grid-cols-1 sm:grid-cols-2 gap-1">{mine.map((p) => <li key={p.id}><Link href={`/praticantes/${p.id}`} className="font-medium text-primary-600 hover:underline">{p.name}</Link></li>)}</ul>}
        </Card>
      )}
    </div>
  );
