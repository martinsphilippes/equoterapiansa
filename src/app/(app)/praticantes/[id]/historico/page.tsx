    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      {hasAny(user, ["practitioners.manage", "sessions.record"]) && (
        <Card title="Adicionar registro à linha do tempo">
          ...
          <p className="text-xs text-ink-500 mt-3">Atendimentos, avaliações e relatórios entram automaticamente. <Link prefetch={false} href={`/praticantes/${id}/atendimentos`} className="text-primary-700 hover:underline">Ver atendimentos</Link></p>
        </Card>
      )}
    </div>
  );
