  ...
  return (
    <div className="space-y-4">
      <PageHeader title={TYPE[a.type]} subtitle={`${isoToBR(a.date)} · ${a.professionalName}`} back={`/praticantes/${id}/avaliacoes`} actions={<>
        {canEdit && <Link href={`/praticantes/${id}/avaliacoes/${aid}?editar=1`} className="inline-flex items-center h-9 px-3 rounded-xl border border-ink-100 bg-white text-sm">Editar</Link>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="rounded-2xl bg-brand-600 text-white p-4"><p className="text-xs uppercase tracking-wide opacity-80">Média geral</p><p className="text-3xl font-semibold">{a.overallAverage ?? "—"}<span className="text-base opacity-80">/{a.scaleMax}</span></p></div>
      </div>
            ...
            {c.items.map((it) => {
              ...
              return (
                <li key={it.id} className="py-2.5 flex flex-wrap items-start justify-between gap-2">
                  ...
                  <span className={`text-sm font-semibold ${s?.score ? "text-brand-800" : "text-ink-300"}`}>{scaleLabel(s?.score ?? null)}</span>
                </li>
              );
            })}
          </ul>
