  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 no-print">
        <div className="flex rounded-xl bg-white border border-ink-100 p-0.5">
          {(["dia", "semana", "mes"] as const).map((v) => <Link key={v} href={q(date, v)} className={`px-3 py-1.5 rounded-lg text-sm capitalize ${view === v ? "bg-brand-600 text-white" : "text-ink-700"}`}>{v === "mes" ? "Mês" : v}</Link>)}
        </div>
        <div className="flex items-center gap-1 ml-auto">
          <Link href={q(prev)} className="px-3 py-1.5 rounded-xl bg-white border border-ink-100">‹</Link>
          <Link href={q(today)} className="px-3 py-1.5 rounded-xl bg-white border border-ink-100 text-sm">Hoje</Link>
          <Link href={q(next)} className="px-3 py-1.5 rounded-xl bg-white border border-ink-100">›</Link>
        </div>
      </div>
      ...
      {view === "mes" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {listDays(start, end).map((d) => (
            <Card key={d} className={`p-0 ${d === today ? "ring-2 ring-brand-300" : ""}`} title={<Link href={q(d, "dia")} className="hover:underline">{weekdayLabel(d)} <span className="text-ink-500 font-normal">{isoToBR(d).slice(0, 5)}</span></Link>}>
              {(byDay.get(d) ?? []).length === 0 ? <p className="text-sm text-ink-300 px-1">Sem atendimentos</p> : (
                <ul className="divide-y divide-ink-100 -m-5">{(byDay.get(d) ?? []).map((a) => <AppointmentRow key={a.id} a={a} canRecord={canRecord} canManage={canManage} linkPractitioner={canOpen} compact />)}</ul>
              )}
            </Card>
          ))}
        </div>
        ...
        <Card className="p-0">
          {appts.length === 0 ? <EmptyState title="Nenhum atendimento neste dia" action={canManage && <LinkButton href={`/agenda/novo?data=${date}`} variant="secondary">Agendar</LinkButton>} /> : (
            <ul className="divide-y divide-ink-100">{appts.map((a) => <AppointmentRow key={a.id} a={a} canRecord={canRecord} canManage={canManage} linkPractitioner={canOpen} />)}</ul>
          )}
        </Card>
      )}
    </div>
...
function MonthGrid({ start, end, byDay, today }: { start: string; end: string; byDay: Map<string, { status: string }[]>; today: string }) {
  ...
  return (
    <div className="rounded-2xl bg-white border border-ink-100 overflow-hidden">
      <div className="grid grid-cols-7 text-center text-xs uppercase tracking-wide text-ink-500 border-b border-ink-100">
        {["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"].map((d) => <div key={d} className="py-2">{d}</div>)}
      </div>
      <div className="grid grid-cols-7">
        {cells.map((d, i) => {
          if (!d) return <div key={`e${i}`} className="min-h-16 border-b border-r border-ink-100 bg-sand-50" />;
          ...
          return (
            <Link key={d} href={`/agenda?visao=dia&data=${d}`} className={`min-h-16 border-b border-r border-ink-100 p-1.5 hover:bg-sand-50 ${d === today ? "bg-brand-50" : ""}`}>
              <span className={`text-xs ${d === today ? "font-bold text-brand-800" : "text-ink-500"}`}>{Number(d.slice(8, 10))}</span>
              <div className="mt-1 flex flex-wrap gap-0.5">
                {pending > 0 && <span className="text-[10px] rounded bg-sky-100 text-sky-800 px-1">{pending}</span>}
                {done > 0 && <span className="text-[10px] rounded bg-brand-100 text-brand-800 px-1">{done}</span>}
