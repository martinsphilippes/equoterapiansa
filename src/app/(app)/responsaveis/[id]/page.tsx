        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Card title="Praticantes">
            {practitioners.length === 0 ? <p className="text-sm text-ink-500">Nenhum praticante vinculado.</p> : (
              <ul className="divide-y divide-ink-100">{practitioners.map((p) => <li key={p.id} className="py-2"><Link href={`/praticantes/${p.id}`} className="font-medium hover:underline">{p.name}</Link></li>)}</ul>
            )}
          </Card>
        </div>
