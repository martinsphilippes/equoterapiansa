...
import { DEFAULT_ORG_ID, enterOrg, withOrgScope } from "@/lib/db/org-context";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const now = Date.now();
    const orgName = body.orgName?.trim() || "Equoterapia";
    await Collections.organizations().doc(DEFAULT_ORG_ID).set({
      id: DEFAULT_ORG_ID, name: orgName, active: true, createdAt: now, createdBy: user.uid,
    });
    await Collections.users().doc(user.uid).set({
      id: user.uid, email: body.email, name: body.name, role: "owner", orgId: DEFAULT_ORG_ID, orgIds: [],
      permissions: DEFAULT_PERMISSIONS.owner, active: true, createdAt: now, updatedAt: now,
    });
    await seedDefaults(orgName);
    ...
    await logRef.set({
      id: logRef.id, action: "setup", entity: "system", entityId: "setup", userId: user.uid, userName: body.name, at: now, details: { indexes },
    });
